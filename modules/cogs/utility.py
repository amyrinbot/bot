...
class Utility(commands.Cog):
    ...
    async def urbandictionary(
        ...
        resp = await self.bot.session.get(
            URBAN_DICTIONARY_API_URL, params={"term": term}
        )
    ...
    @command(
        ...
        examples=["{prefix}nginx remove qtD8v7H2uBV3ceLv.jpg"],
        hidden=True,
    )
    @commands.is_owner()
    async def nginx_remove(
        self,
        ctx: commands.Context,
        name: str = commands.param(description="Name for the file"),
    ):
        try:
    ...
    @command(
        ...
        aliases=["fIPASDHFUONAODJGInAKPÖFnASODfBNSDPGJKAODFHJSKLÖÄGNPSDHUIG"],
        examples=[
            "{prefix}wait",
    ...
    async def task_list(self, ctx):
        command_tasks = [
            ...
            if task.get("user") == ctx.author.id
            and name != getattr(ctx, "_task_name", None)
        ]

        if not command_tasks:
            return await ctx.reply(
                "There are no command tasks running under your name."
            )
    ...
    async def task_cancel(
        self,
        ctx,
        name: str = commands.param(description="The name of the command task."),
    ):
